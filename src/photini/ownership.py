...
class OwnerTab(QtWidgets.QWidget):
    @staticmethod
    def tab_name():
    ...
    def data_form(self):
        ...
        widgets['creator'].setToolTip('<p>' + self.tr(
            'Enter the name of the person that created this image.') + '</p>')
        form.layout().addRow(self.tr('Creator'), widgets['creator'])
        # creator title
        ...
        widgets['creator_title'].setToolTip('<p>' + self.tr(
            'Enter the job title of the person listed in the'
            ' Creator field.') + '</p>')
        form.layout().addRow(
            self.tr("Creator's Jobtitle"), widgets['creator_title'])
        # credit line
        widgets['credit_line'] = SingleLineEdit(
            'credit_line', spell_check=True,
            length_check=ImageMetadata.max_bytes('credit_line'))
        widgets['credit_line'].setToolTip('<p>' + self.tr(
            'Enter who should be credited when this image is'
            ' published.') +'</p>')
        form.layout().addRow(self.tr('Credit Line'), widgets['credit_line'])
        # copyright
        widgets['copyright'] = LangAltWidget(
            'copyright', multi_line=False, spell_check=True,
            length_check=ImageMetadata.max_bytes('copyright'))
        widgets['copyright'].setToolTip('<p>' + self.tr(
            'Enter a notice on the current owner of the'
            ' copyright for this image, such as "©2008 Jane Doe".') + '</p>')
        form.layout().addRow(self.tr('Copyright Notice'), widgets['copyright'])
        ## usage information
        ...
        widgets['rights/UsageTerms'].setToolTip('<p>' + self.tr(
            'Enter instructions on how this image can legally'
            ' be used.') + '</p>')
        rights_group.layout().addRow(
            self.tr('Usage Terms'), widgets['rights/UsageTerms'])
        # web statement of rights
        widgets['rights/WebStatement'] = RightsDropDown('rights/WebStatement')
        rights_group.layout().addRow(
            self.tr('Web Statement'), widgets['rights/WebStatement'])
        # licensor URL
        widgets['rights/LicensorURL'] = SingleLineEdit('rights/LicensorURL')
        widgets['rights/LicensorURL'].setToolTip('<p>' + self.tr(
            'URL for a licensor web page. May facilitate licensing'
            ' of the image.') + '</p>')
        rights_group.layout().addRow(
            self.tr('Licensor URL'), widgets['rights/LicensorURL'])
        form.layout().addRow(self.tr('Rights'), rights_group)
        # special instructions
        widgets['instructions'] = SingleLineEdit(
            'instructions', spell_check=True,
            length_check=ImageMetadata.max_bytes('instructions'))
        widgets['instructions'].setToolTip('<p>' + self.tr(
            'Enter information about embargoes, or other'
            ' restrictions not covered by the Rights Usage Terms field.'
            ) + '</p>')
        form.layout().addRow(self.tr('Instructions'), widgets['instructions'])
        ...
        widgets['contact_info/CiEmailWork'].setToolTip('<p>' + self.tr(
            'Enter the work email address(es) for the person'
            ' that created this image, such as name@domain.com.') + '</p>')
        contact_group.layout().addRow(
            self.tr('Email(s)'), widgets['contact_info/CiEmailWork'])
        # URLs
        widgets['contact_info/CiUrlWork'] = SingleLineEdit(
            'contact_info/CiUrlWork')
        widgets['contact_info/CiUrlWork'].setToolTip('<p>' + self.tr(
            'Enter the work Web URL(s) for the person that'
            ' created this image, such as http://www.domain.com/.') + '</p>')
        contact_group.layout().addRow(
            self.tr('Web URL(s)'), widgets['contact_info/CiUrlWork'])
        # phone numbers
        widgets['contact_info/CiTelWork'] = SingleLineEdit(
            'contact_info/CiTelWork')
        widgets['contact_info/CiTelWork'].setToolTip('<p>' + self.tr(
            'Enter the work phone number(s) for the person'
            ' that created this image, using the international format,'
            ' such as +1 (123) 456789.') + '</p>')
        contact_group.layout().addRow(
            self.tr('Phone(s)'), widgets['contact_info/CiTelWork'])
        # address
        widgets['contact_info/CiAdrExtadr'] = MultiLineEdit(
            'contact_info/CiAdrExtadr',
            length_check=ImageMetadata.max_bytes('contact_info'),
            spell_check=True)
        widgets['contact_info/CiAdrExtadr'].setToolTip('<p>' + self.tr(
            'Enter address for the person that created this image.') + '</p>')
        contact_group.layout().addRow(
            self.tr('Address'), widgets['contact_info/CiAdrExtadr'])
        # city
        widgets['contact_info/CiAdrCity'] = SingleLineEdit(
            'contact_info/CiAdrCity', spell_check=True)
        widgets['contact_info/CiAdrCity'].setToolTip('<p>' + self.tr(
            'Enter the city for the address of the person'
            ' that created this image.') + '</p>')
        contact_group.layout().addRow(
            self.tr('City'), widgets['contact_info/CiAdrCity'])
        # postcode
        widgets['contact_info/CiAdrPcode'] = SingleLineEdit(
            'contact_info/CiAdrPcode')
        widgets['contact_info/CiAdrPcode'].setToolTip('<p>' + self.tr(
            'Enter the postal code for the address of the person'
            ' that created this image.') + '</p>')
        contact_group.layout().addRow(
            self.tr('Postal Code'), widgets['contact_info/CiAdrPcode'])
        # region
        widgets['contact_info/CiAdrRegion'] = SingleLineEdit(
            'contact_info/CiAdrRegion', spell_check=True)
        widgets['contact_info/CiAdrRegion'].setToolTip('<p>' + self.tr(
            'Enter the state for the address of the person'
            ' that created this image.') + '</p>')
        contact_group.layout().addRow(
            self.tr('State/Province'), widgets['contact_info/CiAdrRegion'])
        # country
        widgets['contact_info/CiAdrCtry'] = SingleLineEdit(
            'contact_info/CiAdrCtry', spell_check=True)
        widgets['contact_info/CiAdrCtry'].setToolTip('<p>' + self.tr(
            'Enter the country name for the address of the person'
            ' that created this image.') + '</p>')
        contact_group.layout().addRow(
            self.tr('Country'), widgets['contact_info/CiAdrCtry'])
        form.layout().addRow(Label(
            self.tr('Creator Contact Information'), lines=3), contact_group)
    ...
    def _edit_template(self, template):
        ...
        widgets['copyright'].setToolTip(
            widgets['copyright'].toolTip() + '<p>' + self.tr(
